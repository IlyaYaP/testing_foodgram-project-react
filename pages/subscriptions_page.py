...
class SubscriptionsPage(BasePage):
    def subscription(self):
        ...
        with allure.step('Подписываемся на автора.'):
            author_name = self.browser.find_element(
                        *SubscriptionsLocators.AUTHOR)
            author_name.click()
            button_sub = self.browser.find_element(
                        *SubscriptionsLocators.SUBSCRIPTIONS_BUTTON)
            try:
    ...
    def unsubscribe(self):
        ...
        with allure.step('Отписываемся от автора.'):
            self.go_to_subscriptions_page()
            try:
                unsubscribe_button = self.browser.find_element(
                                   *SubscriptionsLocators.UNSUBSCRIBE_BUTTON)
    ...
    def should_be_subscription(self):
        ...
        with allure.step('Проверяем, что мы успешно подписались на автора.'):
            ...
            assert self.is_element_present(
                *SubscriptionsLocators.AUTHOR), 'The subscription is not presented'
    ...
    def not_should_be_subscription(self):
        ...
        with allure.step('Проверяем, что мы успешно отписались от автора.'):
            ...
            assert self.is_not_element_present(
                *SubscriptionsLocators.AUTHOR), 'The subscription is presented'
    ...
    def is_disappeared_subscribe(self):
        ...
        with allure.step('Проверяем, что мы успешно отписались от автора.'):
            ...
            assert self.is_disappeared(
                *SubscriptionsLocators.AUTHOR_UNSUBSCRIBE)
